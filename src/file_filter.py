...
class FileFilter:
    def __init__(self , file_name :str , lifetime : int , date_pattern : str , glob_pattern : str = ""):
    ...
    def is_valid(self, log_errors : bool = True) -> bool:
        result  = self.glob_filter() and self.pattern_filter() and self.value_filter() and self.date_filter() and self.lifetime_filter()
...
def validate_a( args):
    validate(args.driveId , args.folderId , int(args.lifetime) , args.pattern , bool(args.force))
    return 
def validate( drive_id : str, folder_id : str , lifetime : int , pattern : str , force : bool):
    # error checks
    if drive_id == "NULL":
        logging.error("Please provide a valid drive id , either directly by argument , or modify the configuration file")
        return
    if folder_id == "NULL":
        logging.error("Please provide a valid folder id , either directly by argument, or modify the configuration file")
        return
    if lifetime < 0:
